...
const views = {
    analysis: `
        ...
        <div id="drop-zone">
            ...
            <p class="subtitle">Supports MP3, WAV, and AIFF files with recursive folder scanning</p>
        </div>
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Note: Removed instrumentation.onStart handler - status updates handled by jobProgress events
    // to ensure UI only shows "PROCESSING" when tasks are actually running (not queued)
...
function filterTracksOptimized(tracks, filters) {
    ...
    return tracks.filter(track => {
        ...
        for (const [facet, filterSet] of activeFilters) {
            ...
            let match = false;
            if (facet === 'instrument') {
                // Normalize BOTH filter values and track instruments for matching
                // This allows "Brass" filter to match tracks with "Brass (section)"
                const normalizedFilterArray = [...filterSet].map(v => 
                    String(v).toLowerCase().replace(/\s*\(section\)\s*/i, '')
                );
                const normalizedTrackInstArray = [...track._instruments].map(v => 
                    String(v).replace(/\s*\(section\)\s*/i, '')
                );
                // Preserve AND logic: ALL selected instruments must be present in track
                match = normalizedFilterArray.every(v => normalizedTrackInstArray.includes(v));
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.backgroundProcessing) {
                    ...
                    track.creativeStatus = 'WAITING';  // Let jobProgress events update to PROCESSING
                    track.instrumentationState = 'waiting';  // Let jobProgress events update to PROCESSING
                    track.instrumentationDisplay = 'WAITING';  // Let jobProgress events update to PROCESSING
                    console.log(`[Renderer] Technical complete, background phases queued: ${track.fileName}`);
                } else {
...
function updateQueueDisplay() {
    ...
    const getStatusBadge = (status) => {
        const statusClass = status ? status.toLowerCase().replace(/\s+/g, '-') : 'queued';
        ...
        // Override styling for LLM Derived status (orange/warning color)
        if (status === 'LLM DERIVED') {
            return `<span class="status-badge" style="background: #f59e0b; color: white; font-weight: 600;">${label}</span>`;
        }
        
        return `<span class="status-badge status-${statusClass}">${label}</span>`;
    };

    // Count fully completed tracks (all phases COMPLETE)
    const completedCount = currentQueue.filter(t => 
        t.status !== 'SKIP' &&  // Exclude skipped tracks
        t.techStatus === 'COMPLETE' && 
        t.creativeStatus === 'COMPLETE' && 
        (t.instrumentationState === 'complete' || t.instrumentationDisplay === 'COMPLETE') &&
        !(t.hasExistingAnalysis && !allowReanalyze)  // Don't count files being skipped
    ).length;

    let html = `
        <h3>Files to Process (${completedCount} of ${currentQueue.length}${existingCount > 0 ? ` - ${newCount} new, ${existingCount} existing` : ''})</h3>
    ...
    currentQueue.forEach(track => {
        ...
        } else if (track.instrumentationState === 'error' || track.instrumentationDisplay === 'ERROR') {
            // Check for LLM fallback when instrumentation errors
            const hasLlmInstruments = track.creative?.suggestedInstruments?.length > 0 ||
                                      track.analysis?.creative?.suggestedInstruments?.length > 0;
            instrumentationDisplay = hasLlmInstruments ? 'LLM DERIVED' : 'ERROR';
            ...
            <tr ${rowStyle}>
                <td>
                    <a href="#" 
                       ...
                       style="color: #007AFF; text-decoration: none; cursor: pointer;"
                       onmouseover="this.style.textDecoration='underline'" 
                       onmouseout="this.style.textDecoration='none'">
                        ${track.fileName || track.filename || 'Unknown'}
                    </a>
                </td>
    ...
    // Wire up file name clicks to show in Finder
    queueDiv.querySelectorAll('.file-link').forEach(link => {
        link.addEventListener('click', async (e) => {
            e.preventDefault();
            const filePath = e.currentTarget.dataset.path;
            if (filePath) {
                await window.api.searchShowFile(filePath);
            }
        });
    });
...
if (window.api && window.api.onJobProgress) {
    window.api.onJobProgress((event, data) => {
        ...
        if (data.stage === 'technical') {
            ...
            track.creativeStatus = data.status;  // Update UI state
        } else if (data.stage === 'instrumentation') {
            progressStatus[data.trackId].instrumentation = { status: data.status, note: data.note };
            track.instrumentationState = data.status.toLowerCase();  // Update UI state
            track.instrumentationDisplay = data.status;  // Update UI state
        }
        updateQueueDisplay();
    });
}
...
window.api?.onTrackComplete?.((data) => {
    ...
    if (track) {
        ...
        // Normalize analysis data and copy into track object
        if (data.analysis) {
            normalizeAnalysis(data.analysis);
            
            // ADD THIS: Copy analysis data into track object for LLM fallback check
            track.analysis = data.analysis;
            track.creative = data.analysis.creative;
