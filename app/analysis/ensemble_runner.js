...
function analyzeWithEnsemble(audioPath, opts = {}) {
  ...
  // In packaged app, use .venv from source directory (personal use only)
  // This avoids bundling 2.8 GB of Python packages
  const venvPy = app.isPackaged
    ? '/Volumes/ATOM RAID/Dropbox/_Personal Files/12 - AI Vibe Coding/02 - Cursor Projects/04 - Rhythm DNA v4/app/py/.venv/bin/python'
    : path.resolve(__dirname, "../py/.venv/bin/python");
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const child = execFile(venvPy, args, { timeout: timeoutMs, env: pyEnv }, async (err, stdout, stderr) => {
        ...
        // DIAGNOSTIC: Echo Python stdout to main console for diagnostic logging visibility
        if (stdout) {
          const lines = stdout.split('\n').filter(l => l.includes('[DIAGNOSTIC]') || l.includes('[ELECTRONIC'));
          if (lines.length > 0) {
            console.log('[PYTHON-STDOUT-DIAGNOSTIC]', lines.join('\n'));
          }
        }
        ...
        try {
          ...
          const parsed = {
            ...
            electronic_elements: result?.electronic_elements || null,  // v3.1.0: Electronic detection
            __electronic_detection_code_reached__: result?.__electronic_detection_code_reached__ || false,  // Diagnostic flag
...
function mapResultToAnalysis(js) {
  const out = {
    ...
    electronic_elements: js.electronic_elements || null,  // v3.1.0: Electronic/synthesizer detection
  };
