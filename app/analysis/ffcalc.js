...
// v3.0.0: 3-Queue Architecture - Tech, Creative, and Instrumentation run independently
// Each queue has its own concurrency control for optimal resource utilization

// Queue 1: Creative Analysis (Ollama LLM)
let activeCreativeTasks = 0;
const MAX_CREATIVE_CONCURRENCY = 4;
const creativeQueue = [];

// Queue 2: Instrumentation Analysis (Python ML models)
let activeInstrumentationTasks = 0;
const MAX_INSTRUMENTATION_CONCURRENCY = 4;
const instrumentationQueue = [];

// Completion tracker: tracks which stages each file has completed
// Structure: Map<filePath, { tech: bool, creative: bool, instrumentation: bool, data: {} }>
const fileCompletionTracker = new Map();

// Process next Creative task
function processNextCreativeTask() {
  if (activeCreativeTasks >= MAX_CREATIVE_CONCURRENCY || creativeQueue.length === 0) {
    return;
  }
  
  const task = creativeQueue.shift();
  activeCreativeTasks++;
  console.log(`[CREATIVE-QUEUE] Starting task (active: ${activeCreativeTasks}/${MAX_CREATIVE_CONCURRENCY}, queued: ${creativeQueue.length})`);
  
  task()
    .catch(err => {
      console.error('[CREATIVE-QUEUE] Task failed:', err);
    })
    .finally(() => {
      activeCreativeTasks--;
      console.log(`[CREATIVE-QUEUE] Task complete (active: ${activeCreativeTasks}/${MAX_CREATIVE_CONCURRENCY}, queued: ${creativeQueue.length})`);
      processNextCreativeTask();
    });
}

// Process next Instrumentation task
function processNextInstrumentationTask() {
  if (activeInstrumentationTasks >= MAX_INSTRUMENTATION_CONCURRENCY || instrumentationQueue.length === 0) {
    return;
  }
  
  const task = instrumentationQueue.shift();
  activeInstrumentationTasks++;
  console.log(`[INSTRUMENTATION-QUEUE] Starting task (active: ${activeInstrumentationTasks}/${MAX_INSTRUMENTATION_CONCURRENCY}, queued: ${instrumentationQueue.length})`);
  
  task()
    .catch(err => {
      console.error('[INSTRUMENTATION-QUEUE] Task failed:', err);
    })
    .finally(() => {
      activeInstrumentationTasks--;
      console.log(`[INSTRUMENTATION-QUEUE] Task complete (active: ${activeInstrumentationTasks}/${MAX_INSTRUMENTATION_CONCURRENCY}, queued: ${instrumentationQueue.length})`);
      processNextInstrumentationTask();
    });
}

// Enqueue Creative task
function enqueueCreativeTask(taskFn) {
  creativeQueue.push(taskFn);
  console.log(`[CREATIVE-QUEUE] Task queued (active: ${activeCreativeTasks}/${MAX_CREATIVE_CONCURRENCY}, queued: ${creativeQueue.length})`);
  processNextCreativeTask();
}

// Enqueue Instrumentation task
function enqueueInstrumentationTask(taskFn) {
  instrumentationQueue.push(taskFn);
  console.log(`[INSTRUMENTATION-QUEUE] Task queued (active: ${activeInstrumentationTasks}/${MAX_INSTRUMENTATION_CONCURRENCY}, queued: ${instrumentationQueue.length})`);
  processNextInstrumentationTask();
}

// Check if all 3 stages are complete and finalize the file (write CSV, update DB)
async function checkAndFinalizeFile(filePath) {
  const tracker = fileCompletionTracker.get(filePath);
  if (!tracker) {
    console.error('[FINALIZE] No tracker found for:', filePath);
    return;
  }
  
  if (tracker.tech && tracker.creative && tracker.instrumentation) {
    console.log('[FINALIZE] All 3 stages complete for:', path.basename(filePath));
    
    // Extract data from tracker
    const { techData, creativeResult, instrumentationResult, win, dbFolder, settings } = tracker.data;
    const { baseName, probe, hasWav, probes, finalBpm, tempoSource, id3Tags, durSec, dir } = techData;
    
    // Build final analysis object by combining all 3 stages
    const creative = (creativeResult && creativeResult.data) || {};
    
    // Extract instruments from instrumentation result
    // Note: instrumentationResult has instruments directly on it, not under .data
    const instrumentsFromPy = Array.isArray(instrumentationResult?.instruments) ? instrumentationResult.instruments : [];
    
    // Process and finalize instruments
    const analysis = {
      file: baseName,
      path: filePath,
      ...
      tempo_bpm: finalBpm,
      bpm: finalBpm,
      tempo_confidence: probe?.tempoConfidence || null,
      tempo_source: tempoSource,
      tempo_alt_half_bpm: (Number.isFinite(finalBpm) ? Math.round(finalBpm * 0.5) : null),
      tempo_alt_double_bpm: (Number.isFinite(finalBpm) ? Math.round(finalBpm * 2) : null),
      duration_sec: durSec,
      ...
      creative,
      instruments: instrumentsFromPy,
      finalInstruments: instrumentsFromPy,
      instruments_ensemble: {
        used_demucs: Boolean(instrumentationResult?.used_demucs || instrumentationResult?.usedDemucs),
        mode: (instrumentationResult?.used_demucs || instrumentationResult?.usedDemucs) ? 'stems' : 'mix-only',
        decision_trace: instrumentationResult?.decision_trace || null,
        electronic_elements: instrumentationResult?.electronic_elements || null  // v3.1.0: Electronic detection
      }
    };
    ...
    if (shouldWriteCsv(settings)) {
      try {
        ...
        const csvRows = [
          ...
          ['', ''],
          ['--- Instrumentation (ML Detected) ---', ''],
          ['Detected Instruments', (analysis.finalInstruments || []).join(', ')],
          ['Electronic Elements', (() => {
            const elec = analysis.instruments_ensemble?.electronic_elements;
            if (!elec || !elec.detected) return 'No';
            const conf = elec.confidence ? ` (confidence: ${elec.confidence})` : '';
            return `Yes${conf}`;
          })()],
          ['', ''],
          ['--- Creative Analysis (LLM) ---', ''],
          ['Genre', (analysis.creative?.genre || []).join(', ')],
          ['Mood', (analysis.creative?.mood || []).join(', ')],
          ['Theme', (analysis.creative?.theme || []).join(', ')],
          ['Suggested Instruments', (analysis.creative?.suggestedInstruments || analysis.creative?.instrument || []).join(', ')],
    ...
    if (dbFolder && settings) {
      try {
        ...
        console.log('[FINALIZE] DB updated:', dbResult.key, 'Total tracks:', dbResult.total);
        if (settings.autoUpdateDb) {
          const criteriaResult = await DB.rebuildCriteria(dbPaths);
          console.log('[FINALIZE] Criteria auto-updated:', criteriaResult.counts);
        }
      } catch (e) {
        console.error('[FINALIZE] DB upsert failed:', e);
      }
    }
    
    // Send completion event
    console.log('[FINALIZE] Analysis complete for:', baseName);
    ...
    // Clean up tracker
    fileCompletionTracker.delete(filePath);
  } else {
    console.log(`[FINALIZE] Waiting for stages - Tech: ${tracker.tech}, Creative: ${tracker.creative}, Instrumentation: ${tracker.instrumentation}`);
  }
}
...
async function runInstrumentationAnalysis(filePath, win, audioHints = {}, creativeData = null) {
  try {
    console.log('[INSTRUMENTATION] ===== CODE VERSION: befb833c-FIXED-2025-10-03 =====');
    ...
    // Note: Status updates are sent by the orchestration layer (SEQUENTIAL/CONCURRENT modes)
    // to ensure accurate UI state based on when Instrumentation actually starts
...
async function analyzeMp3(filePath, win = null, model = 'qwen3:8b', dbFolder = null, settings = null) {
  ...
  // === v3.0.0: Technical complete - enqueue to Creative AND Instrumentation queues independently ===
  console.log('[ORCHESTRATION] Technical complete (BPM + ID3), enqueueing Creative and Instrumentation');
  ...
  // Initialize completion tracker for this file
  fileCompletionTracker.set(filePath, {
    tech: true,  // Technical just completed
    creative: false,
  ...
  if (win) {
    ...
    win.webContents.send('jobProgress', {
      ...
      stage: 'creative',
      status: 'WAITING',
      note: 'Queued for creative analysis...'
    });
    win.webContents.send('jobProgress', {
      trackId: filePath,
      stage: 'instrumentation',
      status: 'WAITING',
      note: 'Queued for instrumentation...'
    });
  }
  
  // Enqueue to BOTH queues independently
  enqueueCreativeTask(() => runCreativeTask(filePath, win, model, techData));
  enqueueInstrumentationTask(() => runInstrumentationTask(filePath, win, techData));
...
// v3.0.0: Creative task runner (runs independently in Creative queue)
async function runCreativeTask(filePath, win, model, techData) {
  const { baseName, finalBpm, probes } = techData;
  
  console.log('[CREATIVE-TASK] Starting for:', baseName);
  ...
  if (win && !win.isDestroyed()) {
    win.webContents.send('jobProgress', {
      ...
      stage: 'creative',
      status: 'PROCESSING',
      note: 'Creative analysis started...'
    });
  }
  
  try {
    const creativeResult = await runCreativeAnalysis(
      baseName,
      finalBpm,
      model,
      probes.hints || {}
    ...
    console.log('[CREATIVE-TASK] Complete for:', baseName);
    
    // Store Creative result in tracker
    const tracker = fileCompletionTracker.get(filePath);
    if (tracker) {
      tracker.creative = true;
      tracker.data.creativeResult = creativeResult;
      ...
      if (win && !win.isDestroyed()) {
        win.webContents.send('jobProgress', {
          ...
          note: creativeResult.error ? 'Creative analysis error' : 'Creative analysis complete'
        });
      }
      
      // Check if all 3 stages are complete
      checkAndFinalizeFile(filePath);
    }
  } catch (err) {
    console.error('[CREATIVE-TASK] Failed for:', baseName, err);
    
    // Mark as complete even if failed (don't block finalization)
    const tracker = fileCompletionTracker.get(filePath);
    if (tracker) {
      tracker.creative = true;
      tracker.data.creativeResult = { error: true };
      checkAndFinalizeFile(filePath);
    }
  }
}

// v3.0.0: Instrumentation task runner (runs independently in Instrumentation queue)
async function runInstrumentationTask(filePath, win, techData) {
  const { baseName, probes } = techData;
  
  console.log('[INSTRUMENTATION-TASK] Starting for:', baseName);
  ...
  if (win && !win.isDestroyed()) {
    win.webContents.send('jobProgress', {
      ...
      stage: 'instrumentation',
      status: 'PROCESSING',
      note: 'Instrumentation analysis started...'
    });
  }
  
  try {
    const instrumentationResult = await runInstrumentationAnalysis(
      filePath,
      win,
      probes.hints || {},
      null  // No creative hints - independent execution
    );
    
    console.log('[INSTRUMENTATION-TASK] Complete for:', baseName);
    
    // Store Instrumentation result in tracker
    const tracker = fileCompletionTracker.get(filePath);
    if (tracker) {
      tracker.instrumentation = true;
      tracker.data.instrumentationResult = instrumentationResult;
      ...
      if (win && !win.isDestroyed()) {
        win.webContents.send('jobProgress', {
          trackId: filePath,
          stage: 'instrumentation',
          status: instrumentationResult.error ? 'ERROR' : 'COMPLETE',
          note: instrumentationResult.error ? 'Instrumentation error' : 'Instrumentation complete'
        });
      }
    ...
    console.error('[INSTRUMENTATION-TASK] Failed for:', baseName, err);
    
    // Mark as complete even if failed (don't block finalization)
    const tracker = fileCompletionTracker.get(filePath);
    if (tracker) {
      tracker.instrumentation = true;
      tracker.data.instrumentationResult = { error: true };
      checkAndFinalizeFile(filePath);
    }
  }
}
...
async function completeAnalysisInBackground(filePath, win, model, dbFolder, settings, techData) {
  ...
  // Send PROCESSING status now that task is actually starting
  if (win && !win.isDestroyed()) {
    win.webContents.send('jobProgress', {
      trackId: filePath,
      stage: 'creative',
  ...
  if (INSTRUMENTATION_MODE === "SEQUENTIAL") {
    ...
    // Send Instrumentation PROCESSING status
    if (win && !win.isDestroyed()) {
      win.webContents.send('jobProgress', {
        trackId: filePath,
        stage: 'instrumentation',
    ...
    // Concurrent (default): Creative and Instrumentation run in parallel
    // Instrumentation doesn't wait for Creative (creative hints are optional fallback only)
    console.log('[ORCHESTRATION] Running CONCURRENT: Creative || Instrumentation');
    
    // Send PROCESSING status for both stages immediately
    if (win && !win.isDestroyed()) {
      win.webContents.send('jobProgress', {
        trackId: filePath,
        stage: 'instrumentation',
    ...
    // Run both in parallel - Instrumentation starts immediately, doesn't wait for Creative
    const [creativeRes, instrumentationRes] = await Promise.all([
      runCreativeAnalysis(baseName, finalBpm, model, probes.hints || {}),
      runInstrumentationAnalysis(filePath, win, probes.hints || {}, null)  // No creative hints in parallel mode
    ]);
    
    creativeResult = creativeRes;
    instrumentationResult = instrumentationRes;
    console.log('[ORCHESTRATION] Creative and Instrumentation complete');
  }
  ...
  // DIAGNOSTIC: Check if instrumentationResult has electronic_elements
  console.log('[DEBUG-INSTR] Has electronic_elements?', !!instrumentationResult?.electronic_elements);
  console.log('[DEBUG-INSTR] electronic_elements value:', JSON.stringify(instrumentationResult?.electronic_elements, null, 2));
