...
def analyze(audio_path: str, use_demucs: bool = True, diag: bool = False, creative_hints: dict = None) -> Dict[str, Any]:
    ...
    # Instantiate models once - use GPU (MPS) on Apple Silicon for better performance
    import torch
    panns_device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"[ENSEMBLE] PANNs using device: {panns_device}", flush=True)
    panns = PANNsAT(device=panns_device)
    ...
    if use_demucs:
        ...
        try:
            ...
            # Use GPU (MPS) on Apple Silicon for better parallel performance
            htsat_device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
            print(f"[ENSEMBLE] HTS-AT using device: {htsat_device}", flush=True)
            
            htsat_scores = _infer_htsat_clipwise(wav_t, sr, htsat_device)
    ...
    try:
        ...
        if not used_demucs:
            ...
            # Note: Removed _boost_mix_only_woodwinds_v1 call - had overly permissive
            # thresholds (rollup_min_mean_any=0.00035) causing false positives.
            # Proper woodwind detection with context gates runs later at line 4967.
    except Exception as e:
        # Never break pipeline if booster hiccups
        try:
            out.setdefault("decision_trace", {})
